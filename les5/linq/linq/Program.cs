...
public class Program
{
    ...
    {
        foreach (Movie movie in Movies.OrderByDescending(movie => movie.Year))
        {
    ...
    {
        Console.WriteLine(Movies.Count(movie => movie.Genre == "Sci-Fi" && movie.Director == "Peter Weir"));
    }
    ...
    {
        Console.WriteLine(Movies.Select(movie => movie.Year).Contains(1997));
    }

    static void Opdracht6() // Van welke regisseur is de film 'One Flew over the Cuckoo's Nest'?
    {
        Console.WriteLine(Movies.Where(m => m.Title.Equals("One Flew Over the Cuckoo's Nest")).SingleOrDefault().Director);
    }
